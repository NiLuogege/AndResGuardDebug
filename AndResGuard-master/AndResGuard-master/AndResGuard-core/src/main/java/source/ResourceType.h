 ...
 * style table is an array of ResStringPool_span structures.
 ...
 字符串池的定义。该块的数据是uint32_t的数组，相对于* stringsStart，它们提供了到池中的索引。
 在stringsStart处，所有UTF-16字符串*串联在一起；每个都以字符串*长度的uint16_t开头，
 每个都以0x0000终止符结尾。如果字符串> 32767个字符，则将长度的高位设置为*将这15位作为高位字，
 然后紧跟另一个* uint16_t，其中包含低位字。 * *如果styleCount不为零，
 则紧接着* uint32_t索引数组进入字符串表后，是紧接stylesStart开始的另一个索引数组*进入样式表。
 *样式表中的每个条目都是ResStringPool_span结构的数组。
